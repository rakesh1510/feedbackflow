(function () {
  ...
  wrap.innerHTML = `
    <button id="ff-btn" style="position:fixed;bottom:20px;right:20px;z-index:99999;background:#111827;color:#fff;border:none;border-radius:999px;padding:12px 18px;cursor:pointer;box-shadow:0 8px 20px rgba(0,0,0,.2)">Feedback</button>
    <div id="ff-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.35);z-index:99998">
      <div style="max-width:420px;margin:8vh auto;background:#fff;border-radius:16px;padding:20px;font-family:Arial,sans-serif">
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end">
          ...
          <button id="ff-submit" style="background:#111827;color:#fff;border:none;border-radius:10px;padding:10px 14px;cursor:pointer">Send</button>
        </div>
      </div>
    </div>`;
  document.body.appendChild(wrap);
  document.getElementById("ff-btn").onclick = () => document.getElementById("ff-modal").style.display = "block";
  document.getElementById("ff-close").onclick = () => document.getElementById("ff-modal").style.display = "none";
  document.getElementById("ff-submit").onclick = async () => {
    const payload = {
      site_key: siteKey,
    ...
    if (!payload.message.trim()) { alert("Please enter your feedback."); return; }
    const res = await fetch(apiUrl, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify(payload) });
    const data = await res.json();
    if (data.status === "success") { alert("Thank you for your feedback!"); document.getElementById("ff-modal").style.display = "none"; document.getElementById("ff-message").value = ""; }
